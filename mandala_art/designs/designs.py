...
from mandala_art.designs.turtle_math import arc, deg2rad
...
def outer1wpetals(radial, lengths, turt, col1, col2, offset=0):
    ...
    for _ in range(18):
        ...
        petal1dotted(
            turt,
            tuple(map(lambda x: int(x), list(turtle.bgcolor()))),
            len(lengths),
            lengths,
            col1,
        )
...
def outer2(turt, base=400, R1=70, R2=50, a1=70, a2=20, offset=0, width=4):
    ...
    s = (
        base * tan(deg2rad(10))
        - R1 * (1 - cos(deg2rad(theta)))
        - x * sin(deg2rad(theta - a2 / 2))
    )
    S3 = s / cos(deg2rad(a3))
...
def petal1(
    ...
    num_petals,
    length,
    fillcol=np.array(turtle.bgcolor()).astype(int),
    angle=20,
    width=1,
):
    """
    ...
    if type(length) == list:
        if len(length) != num_petals:
            raise NameError(
                "LengthDimensionalityError: length should either be a single number or a list of numbers equal to the number of petals"
            )
...
def petal1dotted(
    ...
    num_petals,
    length,
    fillcol=(tuple(map(lambda x: int(x), list(turtle.bgcolor())))),
    angle=20,
    width=1,
):
    """
